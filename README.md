...
| **DB の自動更新** | 起動時にテーブルの作成・スキーマ変更を自動で適用。アップデート時に SQL を手で実行する必要なし |
...
テーブルの作成や、バージョンアップに伴うスキーマ変更は、**アプリの起動時に自動で適用**されます（[データベースの自動更新](#データベースの自動更新)を参照）。手動で SQL を実行する必要はありません。
...
新しいバージョンでテーブル定義が変わっても、**利用者は SQL を実行する必要がありません。** Docker ならイメージを、それ以外なら発行したバイナリを更新して再起動するだけで、データベースが新しいバージョンに合わせて更新されます。

テーブル定義の変更は [RadiCore/Migrations/](RadiCore/Migrations/) に番号付きの SQL ファイル（マイグレーション）として追加していき、アプリに埋め込んで配布します。起動時には [DatabaseMigrator.cs](RadiCore/Infrastructure/DatabaseMigrator.cs) が次の順で処理します。

1. 他のプロセスと同時に実行しないよう、PostgreSQL のアドバイザリロックを取得する
2. 適用履歴テーブル `schema_migrations` が無ければ作成する
3. まだ適用していないマイグレーションを**番号順に 1 つずつ**、個別のトランザクションで実行し、適用したバージョンを `schema_migrations` に記録する
4. ロックを解放し、スケジューラ（番組表更新・録音）の起動へ進む

スケジューラより先に実行するため、番組表更新や録音の途中でテーブル定義が変わることはありません。

| 状況 | 動作 |
|---|---|
| 空のデータベース（初回起動） | すべてのマイグレーションを適用し、テーブルとエリア名の初期データを作成する |
| 適用済みのデータベース | 追加されたマイグレーションだけを適用する。無ければ何もしない |
| マイグレーション導入前から稼働しているデータベース（`reservations` はあるが `schema_migrations` が無い） | 初期スキーマ（`0001_baseline`）を適用済みとして記録し、`0002` 以降を適用する |
| マイグレーションの SQL が失敗した | そのマイグレーションはロールバックされ、**アプリは起動を中止する**。テーブル定義が中途半端なまま録音・番組表更新を動かさないため |
| データベースに接続できない | マイグレーションを行わずに起動を続ける（`/healthz` で DB 到達性を確認できる） |

> [!NOTE]
> データベースの変更は元に戻せません。アップデート前に `pg_dump` でバックアップを取っておくと、問題が起きたときに以前の状態へ戻せます。
>
> 初回起動時に `schema_migrations` テーブルを作成するため、接続ユーザーには `public` スキーマへのテーブル作成権限が必要です（PostgreSQL 15 以降、DB の所有者以外は既定で作成できません）。

新しいマイグレーションを追加する手順は[開発 › マイグレーションの追加](#マイグレーションの追加)を参照してください。
...
仕組みは[動作の仕組み › データベースの自動更新](#データベースの自動更新)を参照してください。現在のマイグレーションは以下です。
