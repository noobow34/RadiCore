...
タグ名は **`v` + 数字 3 つ**（`メジャー.マイナー.パッチ`）である必要があります。ワークフローの起動条件（`v*.*.*`）と、タグ名からイメージのタグを組み立てる処理が、この形式を前提にしているためです。

| タグ名 | 可否 | 備考 |
|---|:---:|---|
| `v1.0.0` | ✅ | |
| `v2026.9.18` | ✅ | 日付をバージョンにする場合はこの形。`9` を `09` と書くと無効なので注意 |
| `v20260918` | ❌ | 区切りが無くワークフローが起動しません |
| `v1.0` | ❌ | 数字が 3 つ必要です |
