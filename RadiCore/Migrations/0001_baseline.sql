--
-- 0001_baseline : 初期スキーマ
--
-- 2026-09 時点の docs/schema.sql（pg_dump --schema-only）をそのまま固定したもの。
-- このファイルは適用済みの環境があるため**今後は変更しない**。スキーマ変更は新しい番号のファイルで行う。
--
-- マイグレーション導入前から稼働している DB（reservations テーブルが存在する DB）には実行されず、
-- 適用済みとして記録だけされる（Infrastructure/DatabaseMigrator.cs）。
--
